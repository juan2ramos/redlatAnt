<?
	...
	class razones extends entity
	{
		function delete()
		{
			$qid = $this->db->sql_query("SELECT count(*) as num FROM empresas WHERE id_razon=".$this->id);
			$query = $this->db->sql_fetchrow($qid);
			if($query["num"] != 0)
				$this->avisoError("Existen empresas relacionadas con esta razón social.<br>NO SE PUEDE BORRAR.");
