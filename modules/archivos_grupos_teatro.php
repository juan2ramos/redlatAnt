...
class archivos_grupos_teatro extends entity
{
	function insert()
	{
		global $CFG,$ME;
		...
		if($_FILES["archivo"]["size"] != 0 && ($tipo==1 || $tipo==2))
		{
			...
			if($tipo == 2)
			{
				$idgT = $this->getAttributeByName("id_grupos_teatro");
				$idgT = $idgT->get("value");
				$carpeta = "/teatro/audio/".$idgT."/grupo/";
	...
	function delete()
	{
		...
		if($tipo == 2)
		{
			$idOD = $this->getAttributeByName("id_grupos_teatro");
			$idOD = $idOD->get("value");
			$carpeta = "/teatro/audio/".$idOD."/grupo/";
	...
	$entidad =& new archivos_grupos_teatro();
	$entidad->set("db",$db);

	$entidad->set("name","archivos_grupos_teatro");
	$entidad->set("labelModule","Archivos del Artista");
	$entidad->set("table","archivos_grupos_teatro");
...
// ---------- ATRIBUTOS          ----------------
	...
	if(isset($_GET["mode"]) && ($_GET["mode"]=="editar" || $_GET["mode"]=="agregar"))
	{
		...
		$qidNT = $entidad->db->sql_query("SELECT count(*) as num,tipo FROM archivos_grupos_teatro WHERE id_grupos_teatro=".$_GET["id_grupos_teatro"] ." GROUP BY orden");
		...
		if($_GET["mode"]=="editar")
		{
			$qidNT = $entidad->db->sql_query("SELECT tipo FROM archivos_grupos_teatro WHERE id = ".$_GET["id"]);
