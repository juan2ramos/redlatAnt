...
class archivos_obras_danza extends entity
{
	...
	function insert()
	{
		...
		if($_FILES["archivo"]["size"] != 0 && ($tipo==1 || $tipo==2))
		{
			...
			{
				$carpeta = "/danza/audio/";
				$idOD = $this->getAttributeByName("id_obras_danza");
				$idOD = $idOD->get("value");
				$qidG = $this->db->sql_query("SELECT id_grupos_danza FROM obras_danza WHERE id =".$idOD);
				$id_grupo = $this->db->sql_fetchrow($qidG);
				$carpeta .= $id_grupo["id_grupos_danza"]."/obras/";
	...
	$entidad =& new archivos_obras_danza();
	$entidad->set("db",$db);

	$entidad->set("name","archivos_obras_danza");
	$entidad->set("labelModule","Archivos");
	$entidad->set("table","archivos_obras_danza");
  ...
  if(isset($_GET["mode"]) && ($_GET["mode"]=="editar" || $_GET["mode"]=="agregar"))
	{
		...
		$qidNT = $entidad->db->sql_query("SELECT count(*) as num,tipo FROM archivos_obras_danza WHERE id_obras_danza=".$_GET["id_obras_danza"] ." GROUP BY orden");
		...
		if($_GET["mode"]=="editar")
		{
			$qidNT = $entidad->db->sql_query("SELECT tipo FROM archivos_obras_danza WHERE id = ".$_GET["id"]);
