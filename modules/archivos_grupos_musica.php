		...
		if($_FILES["archivo"]["size"] != 0 && ($tipo==1 || $tipo==2))
		{
			...
			//$this->getAttributeByName("url")->set("value",NULL);
			   ...
			   $url = $this->getAttributeByName("url");
				...
				mkdir($dir);
				chmod($dir,0777);
				//$this->avisoError($_FILES["archivo"]["tmp_name"]);
            ...
            $carpeta = "/circulart.org/musica/audio/".$idOD;
			$dir=$carpeta;
            ...
            $carpeta = "/circulart.org/musica/audio/".$idOD."/grupo/";
			$dir=$carpeta;
			// intentar crear el directorio $dir
			if (ftp_mkdir($conn_id, $dir)) {
			 echo "creado con éxito $dir\n";
			...
			$carpeta = "/circulart.org/musica/audio/".$idOD."/grupo/".$this->id;
			$dir=$carpeta;
			// intentar crear el directorio $dir
			if (ftp_mkdir($conn_id, $dir)) {
			ftp_chmod($conn_id, 0777, $dir);	
...
// ---------- ATRIBUTOS          ----------------
	...
	if(isset($_GET["mode"]) && ($_GET["mode"]=="editar" || $_GET["mode"]=="agregar"))
	{
		...
		while($numeroT = $entidad->db->sql_fetchrow($qidNT))
		{
			...
			if($numeroT["tipo"]==1  && $numeroT["num"]<10)
				$tipos[1] = "Imagen";
			elseif($numeroT["tipo"]==2  && $numeroT["num"]<10)
				$tipos[2] = "Audio (.mp3)";
			elseif($numeroT["tipo"]==3 && $numeroT["num"]<10)
