...
GLOBAL $CFG, $ME, $entidad, $db;
  ...
  if($result=$db->sql_fetchrow($qid)){
    echo "<br>\n<br>\nNo se puede volver a enviar en este momento.<br>\nHay un envío en ejecución.<br>\n";
    echo "Si desea monitorear el envío, por favor haga clic <a href=\"" . $urlMonitoreo ."\" TARGET=\"_parent\">aquí</a>.<br>\n";
    return;
  }
  preguntar($frm);
  $entidad->loadValues($frm);
  ...
  $db->sql_query($query);
  echo "Listo.<br>\nSi desea monitorear el envío, por favor haga clic <a href=\"" . $urlMonitoreo ."\" TARGET=\"_parent\">aquí</a>.<br>\n";
...
function listado_obras_artista($frm)
{
	...
	$consulta = "
		SELECT * FROM (
				SELECT 'Danza' as tipo,gd.nombre as grupo,od.obra, (CASE WHEN ad.actividad=1 THEN 'Director' WHEN ad.actividad=2 THEN 'Bailarín' WHEN ad.actividad=3 THEN 'Técnico' END) as actividad
				...
				UNION
				SELECT 'Música' as tipo,gm.nombre as grupo,om.produccion as obra, (CASE WHEN am.actividad=1 THEN 'Músico' WHEN am.actividad=2 THEN 'Ingeniero de sonido' WHEN am.actividad=3 THEN 'Roadie' END) as actividad
				...
				UNION
				SELECT 'Música' as tipo,gm.nombre as grupo,om.produccion as obra, 'Productor' as actividad
				FROM productores_musica pm
				LEFT JOIN obras_musica om ON om.id = pm.id_obras_musica
				LEFT JOIN grupos_musica gm ON gm.id = om.id_grupos_musica
				WHERE pm.id_artista = ".$frm["id_artista"]."
				UNION
				SELECT 'Teatro' as tipo,gt.nombre as grupo,ot.obra, (CASE WHEN at.actividad=1 THEN 'Director' WHEN at.actividad=2 THEN 'Actor/Actriz' WHEN at.actividad=3 THEN 'Técnico' WHEN at.actividad=4 THEN 'Productor' END) as actividad
	...
	if(count($obras) == 0)
	{
		echo "<tr><td align=\"center\"><br><b>La persona no está relacionada con ninguna obra y/o producción.</b></td></tr>";
	}
				...
				<td>	
					<table width=\"100%\" border=\"0\" cellspacing=\"1\" cellpadding=\"2\" bgcolor=\"#999999\" class=\"textobco10\">
						<tr bgcolor='#bfbfac' class='title'>
							...
							<td align=\"center\"><b>Obra / Producción</b></td>
...
function usuarios_artistas($frm)
{
	...
	$nivelesNoAceptados = array("1"=>"Administrador","2"=>"Admin Géneros","3"=>"Admin Artistas");
	$nivelesAceptados = array("4"=>"Básico Grupo Danza","5"=>"Básico Grupo Música","6"=>"Básico Grupo Teatro","7"=>"Total Grupo Danza","8"=>"Total Grupo Música","9"=>"Total Grupo Teatro");
	...
	{
		echo "<tr><td><br>El Nivel de Acceso del Usuario (".$nivelesNoAceptados[$usuario["id_nivel"]].") no permite la relación con Artistas.<br><br>Para relacionarlo con Artistas debe estar en alguno de los siguientes niveles: ".implode(", ",$nivelesAceptados).".</td></tr>";
		echo "<tr>
	...
	while($query = $db->sql_fetchrow($qidR))
	{
		echo "<tr bgcolor='#ffffff'><td>".$query["nombre"]."</td>
			<td widht='20%' align='center'><a href=\"".$CFG->admin_dir."/index.php?mode=borrar_rel_usuarios_artistas&id_usuario=".$usuario["id"]."&campo=".$campo."&tabla=".$tabla."&id_grupo=".$query["id"]."\"><img alt=\"Borrar Relación\" border='0' src='".$CFG->icondir."/borrador.gif'></a></td>
			</tr>";
		$grupos[] = $query["id"];
	}
	...
	if($agregarRelacion)
	{
		...
		echo "
			...
			<tr><td><br><b>Agregar Relación con Artista</b></td></tr>
			<tr>
				<td>
