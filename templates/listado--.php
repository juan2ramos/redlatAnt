          ...
          <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="textobco10">
              <tr> 
                <td colspan="2">
                    ...
                    <tr>
										<?$module=nvl($_GET['module'],'');?>
											<?if($module == "grupos_danza" || $module == "grupos_musica" || $module == "grupos_teatro" || $module == "promotores")
											{
												if($_SESSION[$CFG->sesion_admin]["user"]["id_nivel"] == 1 ){?>
		                      <td><input type="button" style="font-size:8pt" value="Agregar" onClick="agregar()"></td>
											<?}}else{?>
												<td><input type="button" style="font-size:8pt" value="Agregar" onClick="agregar()"></td>
											<?} ?>
                      <td><input type="button" style="font-size:8pt" value="Editar" onClick="editar('editar')"></td>
											<?if($module != "promotores" || $_SESSION[$CFG->sesion_admin]["user"]["id_nivel"] == 1){?>
                      <td><input type="button" style="font-size:8pt" value="Eliminar" onClick="eliminar()"></td>
											<?}?>
                      <td><input type="button" style="font-size:8pt" value="Detalles" onClick="editar('consultar')"></td>
											<?if($_SESSION[$CFG->sesion_admin]["user"]["id_nivel"] == 1){?>
												<td><input type="button" value="Descargar" style="font-size:8pt" onClick="download()"></td>
											<?}?>
											<td width='100%'>
											<?if(in_array($_SESSION[$CFG->sesion_admin]["user"]["id_nivel"],$buscar)){?>
	                      <input type="button" style="font-size:8pt" value="Buscar" onClick="buscar()">
											<?}else { echo "&nbsp;";}?>
											</td>  
										</tr>
                  </table>
