...
if($entidad->name == "archivos_grupos_danza" || $entidad->name == "archivos_grupos_musica" || $entidad->name == "archivos_grupos_teatro" || $entidad->name == "archivos_obras_danza" || $entidad->name == "archivos_obras_musica" || $entidad->name == "archivos_obras_teatro")
{
	if($entidad->name == "archivos_grupos_danza")
	{
		$idg = $entidad->getAttributeByName("id_grupos_danza");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_grupos_danza WHERE id_grupos_danza=".$idg;
	}
	elseif($entidad->name == "archivos_grupos_musica")
	{
		$idg = $entidad->getAttributeByName("id_grupos_musica");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_grupos_musica WHERE id_grupos_musica =".$idg;
	}
	elseif($entidad->name == "archivos_grupos_teatro")
	{
		$idg = $entidad->getAttributeByName("id_grupos_teatro");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_grupos_teatro WHERE id_grupos_teatro =".$idg;
	}
	elseif($entidad->name == "archivos_obras_danza")
	{
		$idg = $entidad->getAttributeByName("id_obras_danza");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_obras_danza WHERE id_obras_danza=".$idg;
	}
	elseif($entidad->name == "archivos_obras_musica")
	{
		$idg = $entidad->getAttributeByName("id_obras_musica");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_obras_musica WHERE id_obras_musica=".$idg;
	}
	elseif($entidad->name == "archivos_obras_teatro")
	{
		$idg = $entidad->getAttributeByName("id_obras_teatro");
		$idg = $idg->get("value");
		$consulta = "SELECT count(*) as num FROM archivos_obras_teatro WHERE id_obras_teatro=".$idg;
	}

	$qidNA = $entidad->db->sql_query($consulta);
	$queryNA = $entidad->db->sql_fetchrow($qidNA);
	if($queryNA["num"]>=11)
		$agregar = false;
}
	...
	?>
              ...
              <tr> 
                <td colspan="2">
                    ...
                    <tr>
											<?if($agregar){?>
                      <td valign="top"><a title="Agregar" href="javascript:agregar()"><img alt="Agregar" src="<?=$CFG->wwwroot . "/iconos/transparente/icon-add.gif"?>" border="0"></a></td>
											<?}?>
                      <td valign="top"><a title="Editar" href="javascript:editar('editar')"><img alt="Editar" src="<?=$CFG->wwwroot . "/iconos/transparente/iconoeditar.gif"?>" border="0"></a></td>
                      <td valign="top"><a title="Eliminar" href="javascript:eliminar()"><img alt="Eliminar" src="<?=$CFG->wwwroot . "/iconos/transparente/icon-erase.gif"?>" border="0"></a></td>
                      <td valign="top"><a title="Detalles" href="javascript:editar('consultar')"><img alt="Detalles" src="<?=$CFG->wwwroot . "/iconos/transparente/file2.gif"?>" border="0"></a></td>
                      <td valign="top"><a title="Buscar" href="javascript:buscar()"><img alt="Buscar" src="<?=$CFG->wwwroot . "/iconos/transparente/search1.gif"?>" border="0"></a></td>
											<td width="100%">
							          	...
							          	<tr bgcolor="#bfbfac">
														...
														<td align="right" nowrap class="style2">
															<?if($entidad->get("labelModule")!="") echo strtoupper($entidad->get("labelModule")); else echo strtoupper($entidad->get("name"))?> - 
														</td>
